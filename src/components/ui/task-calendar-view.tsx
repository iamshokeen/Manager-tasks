...
function CalCard({
  ...
  task: TaskShape
  viewMode: 'priority' | 'dept' | 'assignee'
  onClick: () => void
  syncing: boolean
}) {
...
export function TaskCalendarView({ tasks, onTaskClick, mutate }: TaskCalendarViewProps) {
  ...
  async function handleDragEnd(event: DragEndEvent) {
    ...
    setSyncingId(taskId)
    try {
      const newDueDate = targetId === 'unscheduled'
        ? null
        : new Date(targetId + 'T12:00:00').toISOString()
  ...
  const weekLabel = (() => {
    ...
    const mon = weekDates[0].toLocaleDateString('en-GB', opts)
    const fri = weekDates[4].toLocaleDateString('en-GB', opts)
    return `${mon} – ${fri}, ${weekDates[4].getFullYear()}`
  })()
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center bg-[var(--surface-container)] rounded-md p-0.5">
              <button
                onClick={() => { setWeekOffset(v => v - 1); setExpandedCols(new Set()) }}
                className="p-1.5 hover:bg-card rounded transition-colors cursor-pointer"
                title="Previous week"
              >
                <ChevronLeft size={15} />
              </button>
                ...
                <button
                  ...
                  className={cn(
                    ...
                    viewMode === mode
                      ...
                      : 'text-muted-foreground hover:bg-[var(--surface-container-highest)]'
                  )}
                >
        ...
        <div className="grid grid-cols-6 gap-px bg-[var(--outline-variant)]/20 rounded-xl overflow-hidden shadow-sm">
          ...
          <div className="bg-[var(--surface-container-low)] flex flex-col min-h-[520px]">
            <div className="p-3 flex items-center justify-between">
              ...
              <span className="text-[10px] font-bold bg-[var(--surface-container-highest)] px-1.5 py-0.5 rounded text-muted-foreground">
                {unscheduled.length}
              </span>
            </div>
          ...
          {weekDates.map((date, i) => {
            ...
            return (
              <div
                key={key}
                className="bg-[var(--surface)] flex flex-col border-l border-[var(--outline-variant)]/20"
              >
                {/* Day header */}
                <div className={cn(
                  'p-3',
                  today ? 'bg-primary/10' : 'bg-[var(--surface-container-low)]/60'
                )}>
                  <p className={cn(
                ...
                <CalColumn id={key}>
                  {colTasks.length === 0 ? (
                    <div className="border-2 border-dashed border-[var(--outline-variant)]/30 rounded-md h-16 flex items-center justify-center text-[10px] text-muted-foreground/40 hover:border-primary/40 hover:text-primary/40 transition-colors">
                      Drop here
                    </div>
                  ) : (
                    <>
