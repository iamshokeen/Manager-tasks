...
//
// On create we immediately materialize every occurrence in the horizon
// (endDate or now+1y) as Task rows via syncTemplateTasks, so schedules
// are instantly visible in the task list and calendar.
import { NextResponse } from 'next/server'
import { prisma } from '@/lib/prisma'
import { getSession } from '@/lib/auth'
import { canRoleAsync, getVisibleUserIds } from '@/lib/rbac'
import { initialNextRunAt, syncTemplateTasks, type Frequency } from '@/lib/services/recurring-tasks'
...
export async function POST(req: Request) {
  ...
  try {
    ...
    // Materialize every upcoming occurrence as a Task row now.
    const sync = await syncTemplateTasks(template.id)

    return NextResponse.json({ data: template, sync }, { status: 201 })
  } catch {
